...
from autowrite.handwriting_synthesis.tf.utils import dense_layer, shape
...
class LSTMAttentionCell(tfcompat.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        with tfcompat.variable_scope(
            scope or type(self).__name__, reuse=tfcompat.AUTO_REUSE
        ):
            # lstm 1
    ...
    def output_function(self, state):
        params = dense_layer(
            state.h3, self.output_units, scope="gmm", reuse=tfcompat.AUTO_REUSE
        )
    ...
    def _parse_parameters(self, gmm_params, eps=1e-8, sigma_eps=1e-4):
        ...
        pis = tfcompat.where(pis < 0.01, tf.zeros_like(pis), pis)
        ...
        es = tfcompat.where(es < 0.01, tf.zeros_like(es), es)
