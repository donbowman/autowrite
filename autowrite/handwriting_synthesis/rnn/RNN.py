...
from autowrite.handwriting_synthesis.rnn import LSTMAttentionCell
from autowrite.handwriting_synthesis.rnn.operations import rnn_free_run
from autowrite.handwriting_synthesis.tf import BaseModel
from autowrite.handwriting_synthesis.tf.utils import time_distributed_dense_layer
...
class RNN(BaseModel):
    ...
    def nll(y, lengths, pis, mus, sigmas, rho, es, eps=1e-8):
        ...
        bernoulli_likelihood = tf.squeeze(
            tfcompat.where(tf.equal(tf.ones_like(y_3), y_3), es, 1 - es)
        )
    ...
    def calculate_loss(self):
        ...
        params = time_distributed_dense_layer(
            outputs, self.output_units, scope="rnn/gmm"
        )
        pis, mus, sigmas, rhos, es = self.parse_parameters(params)
        sequence_loss, self.loss = self.nll(
