...
from autowrite.handwriting_synthesis.config import checkpoint_path, prediction_path
from autowrite.handwriting_synthesis.tf.utils import shape
...
class BaseModel(object):
            ...
            while step < self.num_training_steps:
                ...
                if step % self.log_interval == 0:
                    ...
                    if early_stopping_metric < best_validation_loss:
                        logging.info(
                            "Updating best validation loss {} with early stopping metric {}.".format(
                                round(best_validation_loss, 4),
                                round(early_stopping_metric, 4),
                            )
                        )
                    ...
                    if step - best_validation_tstep > self.early_stopping_steps:
                        logging.info(
                            "Stopping early at step {}: Best Validation Step: {} Early Stopping Steps: {}".format(
                        ...
                        if self.restart_idx < self.num_restarts and checkpoint_created:
                            logging.info(
                                "Restarting for the {} time out of {} total restarts.".format(
                                    self.restart_idx, self.num_restarts
                                )
                            )
                            try:
                                ...
                                logging.warn(
                                    "Failed to restore checkpoint; will continue training: {} - {}".format(
    ...
    def update_parameters(self, loss):
        if self.regularization_constant != 0:
            l2_norm = tf.reduce_sum(
                [
                    ...
                    for param in tfcompat.trainable_variables()
                ]
            )
        ...
        logging.info(
            pp.pformat([(var.name, shape(var)) for var in tfcompat.global_variables()])
        )

        logging.info("Trainable parameters:")
        logging.info(
            pp.pformat(
                [(var.name, shape(var)) for var in tfcompat.trainable_variables()]
            )
        )

        logging.info("Trainable parameter count:")
        logging.info(
            str(sum(np.prod(shape(var)) for var in tfcompat.trainable_variables()))
        )
    ...
    def get_optimizer(self, learning_rate, beta1_decay):
        if self.optimizer == "adam":
            ...
            return tfcompat.train.RMSPropOptimizer(
    ...
    def build_graph(self):
        with tf.Graph().as_default() as graph:
            ...
            if self.enable_parameter_averaging:
                self.saver_averaged = tfcompat.train.Saver(
